...
export class BoundedNdjsonDecoder {
	...
	push(chunk: Uint8Array): unknown[] {
		if (this.failed) throw new NdjsonFrameError("decoder is closed after a framing error");
...
export function encodeNdjsonFrame(value: unknown, maxFrameBytes = DEFAULT_MAX_FRAME_BYTES): Buffer {
	let json: string;
	try {
	...
	if (json === undefined) throw new NdjsonFrameError("message is not JSON serializable");
