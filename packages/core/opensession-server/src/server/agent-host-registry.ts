...
function ownershipKey(fence: AgentTurnFence): string {
	return JSON.stringify([fence.sessionId, fence.runId, fence.turnId, fence.generation]);
}
...
export class AgentHostRegistry<T> {
	...
	register(fence: AgentTurnFence, value: T): AgentHostRegistration<T> {
		...
		if (current) {
			const kind = current.fence.generation === fence.generation
				? "duplicate"
				: current.fence.generation > fence.generation ? "stale" : "conflicting";
	...
	unregister(fence: AgentTurnFence, value?: T): boolean {
		...
		if (!current || (value !== undefined && current.value !== value)) return false;
