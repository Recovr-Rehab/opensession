...
class FakeDaytonaClient implements DaytonaClient {
  ...
  async create(input: Parameters<DaytonaClient["create"]>[0]) {
    ...
    return daytona("created", "started", input.labels);
  }
  async get(id: string) { this.throwIfFailed(); return this.resources.get(id); }
  async list() { this.throwIfFailed(); return [...this.resources.values()]; }
  async start(id: string) { this.throwIfFailed(); this.events.push(`start:${id}`); }
  async stop(id: string) { this.throwIfFailed(); this.events.push(`stop:${id}`); }
  async delete(id: string) { this.throwIfFailed(); this.events.push(`delete:${id}`); }
...
describe("DaytonaExecutorProvider", () => {
  ...
  test("lists only complete Open Session Executor metadata", async () => {
    ...
    client.resources.set("legacy", daytona("legacy", "started", { sandbox: "yes" }));
    client.resources.set("bad-generation", daytona("bad-generation", "started", {
  ...
  test("ensure delegates only after identity validation", async () => {
    const calls: string[] = [];
    const { client, provider } = setup(new FakeDaytonaClient(), async (resource) => {
      calls.push(resource.id);
      return { executorId: identity.executorId, workspaceId: resource.workspaceId };
    });
    ...
    client.resources.set(identity.resourceId, daytona(identity.resourceId, "started", {
      ...executorMetadata("daytona", identity),
      [EXECUTOR_METADATA_KEYS.sessionId]: "other",
    }));
    await expect(provider.ensureExecutor(identity)).rejects.toThrow("identity mismatch");
  ...
  test("rejects installer identity mismatch and propagates API failures", async () => {
    ...
    await expect(failed.provider.destroy(identity)).rejects.toThrow("Daytona unavailable");
