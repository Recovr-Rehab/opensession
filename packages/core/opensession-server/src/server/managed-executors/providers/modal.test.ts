...
class FakeModalClient implements ModalClient {
  ...
  async create(input: Parameters<ModalClient["create"]>[0]) {
    ...
    return modal(`modal-${this.creates.length}`, "running", input.tags);
  }
...
describe("ModalExecutorProvider", () => {
  ...
  test("stop terminates and start returns a fresh replacement identity", async () => {
    ...
    await expect(provider.inspect(identity)).resolves.toEqual({ state: "missing" });
    await expect(provider.start(identity)).resolves.toEqual({
      resourceId: "modal-1",
      workspaceId: "workspace-modal-1",
    });
    expect(client.creates[0]?.tags).toEqual(executorMetadata("modal", identity));
  });
  ...
  test("filters malformed and legacy tags", async () => {
    ...
    client.resources.set("legacy", modal("legacy", "running", { sandboxId: "old" }));
    client.resources.set("bad", modal("bad", "running", {
  ...
  test("validates identity before fixed installation", async () => {
    let calls = 0;
    const { client, provider } = setup(new FakeModalClient(), async (resource) => {
      calls += 1;
    ...
    client.resources.set(identity.resourceId, modal(identity.resourceId, "running", {
      ...executorMetadata("modal", identity),
