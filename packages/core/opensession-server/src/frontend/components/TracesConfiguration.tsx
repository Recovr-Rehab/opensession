  ...
  async function connect() {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        await new Promise((r) =>
          setTimeout(r, Math.max(start.pollInterval, 2) * 1000),
        );
        if (!aliveRef.current) return;
    ...
    <SettingsSection className="flex flex-col gap-3 border-0 bg-panel p-4">
      <div>
        <div className="text-item-title font-medium text-fg">
          Your Traces identity
        </div>
        <p className="m-0 mt-0.5 text-supporting text-dim">
          Connect the GitHub account you use on traces.com. Interactive sessions
          you own are published as you, not as a shared bot. Automations are
          never uploaded.
        </p>
      </div>
      ...
      {!cliPresent ? (
        <InlineAlert>
          The traces CLI is not on this server&apos;s PATH. Install it for the
          Open Session service user so sessions can be uploaded.
        </InlineAlert>
      ) : null}
      {namespaceSlug ? (
        <p className="m-0 text-supporting text-dim">
          Publishing into @{namespaceSlug}.
        </p>
      ) : null}
      {me ? (
