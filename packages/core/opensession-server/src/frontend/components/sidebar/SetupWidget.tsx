...
export function SetupWidget({
	...
	return (
		<aside
			...
			className="absolute right-3 bottom-[max(12px,env(safe-area-inset-bottom,0px))] left-3 z-20 rounded-xl bg-popup p-2 smooth-shadow-sm"
			onPointerEnter={() => void setup.refetch()}
			onFocusCapture={() => void setup.refetch()}
		>
			<div className="flex min-h-10 items-center gap-2 pl-2">
				<h2 id="sidebar-setup-title" className="m-0 text-label font-semibold text-fg">
					Get started
				</h2>
			...
			<div className="flex flex-col">
				{visibleItems.map((item) => (
					<button
						key={item.id}
						type="button"
						className="focus-ring flex min-h-10 w-full items-center gap-2 rounded-control px-2 text-left text-label font-medium text-fg transition-[background-color,scale] duration-[var(--dur-micro)] hover:bg-hover active:scale-[0.96] phone:min-h-11"
						onClick={() =>
			...
			<button
				type="button"
				className="focus-ring mt-1 flex min-h-10 w-full items-center gap-1 rounded-control px-2 text-left text-meta font-medium text-dim transition-[background-color,scale] duration-[var(--dur-micro)] hover:bg-hover hover:text-fg active:scale-[0.96] phone:min-h-11"
