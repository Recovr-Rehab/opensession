...
import { fetchFeaturebaseStatus, type FeaturebaseAdmin } from "../lib/api/featurebase";
	...
	return (
		<SettingsSection className="flex flex-col gap-3 border-0 bg-panel p-4">
			<div className="flex items-center justify-between gap-3">
				...
				<Button size="sm" variant="ghost" disabled={loading} onClick={() => void check()}>
					{loading ? "Checking…" : "Check connection"}
				</Button>
			</div>
			{loading && !ok && !error ? <LoadingState>Checking Featurebase</LoadingState> : null}
			...
			{admins.length > 0 ? (
				<div>
					...
					<p className="m-0 mt-0.5 mb-2 text-supporting text-dim">
						Copy your id into FEATUREBASE_ADMIN_ID so replies and notes have an author.
					</p>
					<ul className="m-0 flex list-none flex-col gap-1 p-0">
						{admins.map((admin) => (
							<li key={admin.id || admin.email || admin.name} className="flex items-center justify-between gap-2">
