...
function guideFor(integration: SetupIntegration, publicBaseUrl: string): Guide {
  ...
  switch (integration.id) {
    ...
    case "traces":
      return {
        description:
          "Publish Open Session runs to traces.com as each person's GitHub identity.",
        steps: [
          <>
            Install the Traces CLI on this server and keep it on the service
            user's PATH.
          </>,
          <>
            Invite teammates to the Traces org. Set TRACES_NAMESPACE_SLUG to
            that org slug if it is not the namespace they land in after login.
          </>,
          <>
            Each person opens this dialog while signed into Open Session with
            GitHub, clicks Connect Traces, and authorizes the same GitHub
            account on traces.com.
          </>,
          <>
            Enable Traces, save, and restart. Interactive sessions they own are
            uploaded as them. Automations are never uploaded.
          </>,
        ],
        permissions: [
          <>
            A shared API key cannot author as a person. Do not paste a trk_ key
            for publishing.
          </>,
          <>
            Open Session matches Traces to the GitHub login on the session
            (createdByLogin).
          </>,
        ],
      };
    ...
    case "featurebase":
      return {
        description:
          "Connect Featurebase tickets and feedback posts, and send webhooks to Open Session.",
        steps: [
          <>
            In Featurebase, create an API key with access to tickets, posts, and
            comments.
          </>,
          <>
            Create a webhook for <strong>ticket.created</strong>,{" "}
            <strong>ticket.updated</strong>, <strong>post.created</strong>,{" "}
            <strong>post.updated</strong>,{" "}
            <strong>conversation.user.created</strong>, and{" "}
            <strong>conversation.admin.noted</strong>. Use this endpoint:
            <Value value={url("/featurebase/webhook")} />
          </>,
          <>
            Paste the API key and webhook signing secret into the fields above.
            After you save and restart, reopen this dialog and copy your admin
            id from the connection check.
          </>,
          <>
            Set the portal URL to your public Featurebase origin, for example
            https://support.recovr.com.
          </>,
          <>
            Add the Featurebase Reader MCP in Connections. Keep Writer off
            automations that read customer text.
          </>,
          <>
            Enable Featurebase, save, and restart Open Session. Then send a test
            webhook from Featurebase.
          </>,
        ],
        permissions: [
          <>
            Customer replies and public comments stay human-gated in Open
            Session. Triage automations write internal notes only.
          </>,
          <>
            Set FEATUREBASE_ADMIN_ID to your Featurebase admin id so replies and
      ...
      <Modal.Content widthClassName="max-w-[40rem]">
        ...
        {integration.id === "featurebase" && (
          <FeaturebaseConfiguration
            apiKeyPresent={integration.env.some(
              (envVar) =>
                envVar.name === "FEATUREBASE_API_KEY" && envVar.present,
            )}
          />
        )}
