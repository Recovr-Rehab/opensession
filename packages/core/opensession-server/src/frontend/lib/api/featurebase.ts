...
export async function fetchFeaturebaseStatus(): Promise<{
	...
	try {
		return await request("/featurebase/status", { label: "Featurebase status" });
	} catch (error) {
		return {
			ok: false,
			error: error instanceof Error ? error.message : "Featurebase status failed",
		};
	}
}
...
export async function startFeaturebaseTicketTriage(id: string): Promise<string> {
