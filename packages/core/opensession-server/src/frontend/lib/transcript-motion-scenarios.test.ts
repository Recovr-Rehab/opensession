...
describe("transcript motion scenarios", () => {
	...
	test("updates growing entries without reordering their row", () => {
		...
		const append = scenario.events.find(
			(event) => event.kind === "append-entry" && event.entry.type === "tool_result",
		);
		if (!append || append.kind !== "append-entry") throw new Error("missing tool result");
		state = applyTranscriptMotionEvent(state, append);
		const index = state.entries.findIndex((entry) => entry.id === append.entry.id);
		const update = scenario.events.find(
			(event) => event.kind === "update-entry" && event.id === append.entry.id,
		);
		if (!update || update.kind !== "update-entry") throw new Error("missing update");
