...
/** The current reasoning heading doubles as its loading indicator. Match
 * ChatGPT's quieter wash: the text rests at its normal secondary color while a
 * short, low-contrast band crosses the glyphs. Inline-block is load-bearing:
 * the sweep is sized to the text, not the full transcript column. base.css
 * freezes it for reduced motion. */
