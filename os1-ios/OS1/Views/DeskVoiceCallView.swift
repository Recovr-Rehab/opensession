...
/// The composer's mic, seated immediately left of send — the place a voice
/// control is looked for first, and the reason this exists at all: the Desk
/// header's mic is up in the chrome, nowhere near the input.
///
/// Its own view struct on purpose. `SessionInputBar` must not read call state:
/// per the app's observation rule, a property read there re-evaluates the
/// whole bar, and a running call changes `state` on every turn.
///
/// Nothing renders unless the environment carries an engine, which only the
/// Desk provides — so every other session's composer is untouched.
struct ComposerVoiceButton: View {
    @Environment(DeskVoiceEngine.self) private var engine: DeskVoiceEngine?
    ...
    var body: some View {
        if let engine {
            Button {
                ...
                Image(systemName: engine.active ? "waveform" : "mic")
                    .font(.system(size: 18, weight: .medium))
                    // Lit while a call is live: minimized, this button is the
                    // only sign in the composer that the mic is still open.
                    .foregroundStyle(
                        engine.active ? OS1VisualStyle.accent : OS1VisualStyle.textDim
                    )
                    #if os(iOS)
            ...
            .buttonStyle(.plain)
            .accessibilityLabel(
                engine.active ? "Return to the voice call" : "Start a voice call"
            )
        }
    }
}
