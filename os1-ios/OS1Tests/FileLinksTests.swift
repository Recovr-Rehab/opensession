...
final class FileLinksTests: XCTestCase {
    ...
    override func setUp() async throws {
        FileLinks.register(
            paths: ["src/server/pr.ts", "src/server", "os1-ios/OS1/Views/My File.swift"],
            for: session
        )
    }
    ...
    func testPathWithSpacesIsEncoded() {
        XCTAssertEqual(
            linkify("`os1-ios/OS1/Views/My File.swift`"),
            "[os1-ios/OS1/Views/My File.swift]"
            + "(os1file:os1-ios/OS1/Views/My%20File.swift)"
        )
    }
    ...
    func testRoundTripsThroughTheURL() {
        let linked = linkify("`os1-ios/OS1/Views/My File.swift`")
