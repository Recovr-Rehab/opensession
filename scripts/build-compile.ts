...
function generateEmbedModule(distDir: string, version: string): string {
	...
	names.forEach((name, i) => {
		const ident = `__a${i}`;
		// The embed module lives deep under packages/, the dist at the repo root;
		// resolve the specifier from the module's own directory so the depth of
		// the module never has to be tracked here by hand.
		let spec = relative(dirname(EMBED_MODULE), resolve(distDir, name)).replaceAll("\\", "/");
		if (!spec.startsWith(".")) spec = `./${spec}`;
		lines.push(`import ${ident} from ${JSON.stringify(spec)} with { type: "file" };`);
