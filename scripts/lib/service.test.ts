...
describe.skipIf(!onServiceHost)("systemd unit", () => {
  test("rewrites every host-specific directive", async () => {
    ...
    expect(unit.match(/^ExecStart=(\S+) run packages\/core\/opensession-server\/opensession\.ts$/m)?.[1]).toMatch(/bun(?:\.exe)?$/i);
  });
  ...
  test("PATH carries bun and the engine", async () => {
    ...
    expect(path.replaceAll("\\", "/")).toContain(".opencode/bin");
...
describe.skipIf(!onServiceHost)("launchd plist", () => {
  ...
  test("sources the env file, because launchd has no EnvironmentFile", () => {
    ...
    expect(plist).toMatch(/exec \S*bun(?:\.exe)? run packages\/core\/opensession-server\/opensession\.ts/i);
  });
