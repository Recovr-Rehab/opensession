...
export const DEFAULT_WORKSPACE_MODEL_SETTINGS: WorkspaceModelSettings = {
  presets: [
    ...
    { id: "ultracode", label: "Ultracode", group: "custom", lead: { model: "opencode/anthropic/claude-opus-5", effort: "xhigh" }, instructions: "Plan a workflow for every substantive task instead of working through it turn by turn. Fan the work out with run_workflow, have a separate agent verify each finding, and read the result back with workflow_status. Keep quick questions and single edits in the conversation." },
